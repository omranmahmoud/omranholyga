              ...
              {banners.map(banner => (
                <li key={banner._id} style={{ marginBottom: 24, border: '1px solid #eee', borderRadius: 8, padding: 16, position: 'relative' }}>
                  {editId === banner._id ? (
                    ...
                    <>
                      <img src={banner.image?.startsWith('/uploads/') ? `/api${banner.image}` : resolveImageUrl(banner.image)} alt={banner.title || 'Banner'} style={{ width: 320, height: 120, objectFit: 'cover', borderRadius: 8 }} />
